 ...
 *
 * This endpoint is called by Vercel cron to periodically compute and store
 * wallet position snapshots based on trade activity.
 *
 * Schedule: Every 15 minutes (configurable in vercel.json)
 *
 * Positions are computed by aggregating trades and stored as point-in-time snapshots.
 */

import { NextResponse } from "next/server";
import { syncPositionsForAllMarkets } from "@/services/positions";
import { env } from "@/env";
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    // Sync position snapshots for all active markets
    const result = await syncPositionsForAllMarkets({
      maxMarkets: 50,
      tradeLimit: 1000,
    ...
    const marketsWithPositions = result.markets.filter((m) => m.positionsInserted > 0).length;
    ...
    console.error("[cron/positions] Sync failed:", errorMessage);
